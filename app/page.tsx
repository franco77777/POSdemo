      ...
      <div className="relative w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="p-6 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-4">
            No existe ningún producto registrado con el código:
          </p>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
            ¿Deseas agregar este producto al inventario?
          </p>
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 px-4 py-2.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-medium rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
            >
              Cancelar
            </button>
            <button
...
function SearchProductModal({
  ...
  const startScanning = useCallback(async () => {
    ...
    try {
      ...
      if (!videoRef.current) {
        setScanError("No se pudo acceder al elemento de video");
        setIsScanning(false);
        return;
      }
  ...
  const handleBarcodeDetected = (barcode: string) => {
    ...
    const product = products.find(
      (p) => p.codigoBarra.toLowerCase() === barcode.toLowerCase()
    );

    if (product) {
      // Producto existe - redirigir al perfil
      onClose();
      setSearchQuery("");
      router.push(`/product-profile/${product.id}`);
    } else {
      // Producto no existe - mostrar modal
  ...
  const handleAddProduct = () => {
    ...
    onClose();
    setSearchQuery("");
    // Redirigir a /add-product con el código de barras como parámetro
    if (scannedBarcode) {
      router.push(`/add-product?barcode=${encodeURIComponent(scannedBarcode)}`);
    }
  };
            ...
            <>
              ...
              <div className="p-4 border-b border-slate-200 dark:border-slate-700">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Escribe el nombre del producto o código de barras..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full pl-12 pr-32 py-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
                    autoFocus
                  />
                  {/* Botón de escanear */}
                  <button
                    onClick={startScanning}
                    className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-1.5"
                  >
              ...
              <div className="max-h-[400px] overflow-y-auto">
                {!searchQuery.trim() ? (
                  <div className="p-8 text-center text-slate-500 dark:text-slate-400">
                    ...
                    <p>Comienza a escribir o usa el escáner</p>
                    <p className="text-sm mt-1">
                      Haz clic en "Escanear" para usar la cámara
                    </p>
                  </div>
